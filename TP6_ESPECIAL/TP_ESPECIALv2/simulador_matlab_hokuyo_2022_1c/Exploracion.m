function [v_cmd, w_cmd] = Exploracion(idx, ranges, angles, pose, sampleTime, test)
...
%   addScan detecta cierres de bucle y optimiza según la configuración en slamObj
    ...
    % del proceso de construcción del mapa completado, elimine la condición debajo
     if optimizationInfo.IsPerformed && test && false%&& ~firstTimeLCDetected
            figure(3)
...
%             updatedPG = optimizePoseGraph(slamA);
%             figure
...
if idx > 300 & ~EXIT
condExit = abs( poses(:,1:2) - poses(end,1:2) ) < 0.01;
    if condExit(end-1,:)
        save LarMap.mat occMap
...
if EXIT == true
    ...
    [v_cmd, w_cmd] = SuperBlind(scans, maxLidarRange);
    fprintf("v_cmd: %f \n w_cmd: %f", v_cmd, w_cmd);
end
...
function [v_cmd, w_cmd] = SuperBlind(scans_, rangMax)
    persistent old_idx1
    persistent old_idx2
    persistent v_cmd2
    persistent w_cmd2
...
%Diametro total = 25cm.
    ...
    %find(scans.Ranges < diametro)  %ponerlo como inf.
    offSet = 15/100;
    dw = 0.01;
    dv = 0.01;
    ...
    if ((rangMax-2 - 0.5) < (MaxRang) && (MaxRang <= rangMax-2))
    ...
    if ~(old_idx1 < length(scans.Ranges)/2) && ~(old_idx2 < length(scans.Ranges)/2) && (idx < length(scans.Ranges)/2) 
        ...
        w_cmd2 = sgn*w_cmd2; %sgn*abs(w_reco);
        v_cmd = 0;
    end
    v_cmd2= v_cmd2 + dv;
    w_cmd2 = w_cmd2 + dw;
    
    if (w_cmd2 >= w_reco); w_cmd2=w_reco;end
    if (v_cmd2 >= v_reco); v_cmd2=v_reco;end
