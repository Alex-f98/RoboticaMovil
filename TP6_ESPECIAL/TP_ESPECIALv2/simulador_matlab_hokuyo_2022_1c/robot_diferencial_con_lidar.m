...
use_roomba= true; %false;  % false para desarrollar usando el simulador, true para conectarse al robot real

%% Roomba
if use_roomba   % si se usa el robot real, se inicializa la conexion    
    rosshutdown
    pause(1)
    ipaddress_core = '192.168.0.102'; %'192.168.0.101';
    ipaddress_local = '192.168.0.103'; %'192.168.0.100';  %mi ip en a red TurtleNet
    setenv('ROS_IP', '192.168.0.103'); %'192.168.0.100');
...
initPose = [2.5; 2; -pi/2];         % Pose inicial (x y theta) del robot simulado (el robot pude arrancar en cualquier lugar valido del mapa)

%Elegir que se deafio se desea simular/ la opccion "test" indica si se
%quiere visializar los graficos usados para el informe.
...
%     
    ...
    if simulador == "Vigilancia"
    	[v_cmd, w_cmd] = Vigilancia(idx, pose(:,idx-1), double(ranges), lidar.scanAngles, map, test);
        %[v_cmd, w_cmd]= Vigilancia(idx, map, dd, odom, ranges, rangMax, sampleTime);
    elseif simulador == "Exploracion"
        [v_cmd, w_cmd] = Exploracion(idx, double(ranges), lidar.scanAngles, pose(:,idx-1), sampleTime, test);
    end
