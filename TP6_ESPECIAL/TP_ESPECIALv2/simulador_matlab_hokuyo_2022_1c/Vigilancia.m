...
%        w_cmd = 0.7*( atan2(dy, dx) - estimatedPose(3));
 ...
 % ACA YA ESTOY LOCALIZADO: hacer ruta...  
   ...
   if located && ~exist_path
    ...
    %      planeamiento hacia la meta
           [vel_XYTheta, path] = expectVel(path, sampleTime, 60); %(path, st, time)
           exist_path = true;
           idx_ref = idx;
            if test
               %----------------------------------------------------------------------
...
%        end
   ...
   if (idx > ( idx_ref + length(path) - 1 ) ) && is_orient
    ...
    if abs(estimatedPose(1:2) - puntoB) < 0.1
        w_cmd=0;
        v_cmd=0;
        disp("FIN: Llego al punto B");
        exist_path = true;
    end
end
...
%                           sin(theta)/(d*(2*sin(theta)^2 - 1)), cos(theta)/(d*(2*cos(theta)^2 - 1)) ] );
    ...
    Qp = pinvJ(X_real(3)) * ( Kd.*Xp_deseado + Xe_p );
...
function [v_cmd, w_cmd, is_orient] = orient(pose, goalPose, sampleTime)
    ...
    g_tol= (pi/3.5) ; % grados de tolerancia
